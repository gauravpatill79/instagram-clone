...
const OpenSavedReels = ({ savedReels, setSortedReels }) => {
  ...
  const handlePinReel = async (reel) => {
    if (user) {
      ...
      await updateDoc(userDocRef, { pinnedReels });

      const updatedReels = [
        reel,
        ...savedReels.filter((r) => r.id !== reel.id),
      ];
    ...
    <>
      {savedReels.map((reel) => (
        <GridItem
          ...
          position={"relative"}
          aspectRatio={1 / 1}
          onClick={() => openModalWithReel(reel)}
        >
          <Flex
            opacity={0}
            _hover={{ opacity: 1 }}
            ...
            {selectedReel && (
              ...
              >
                <Flex
                  ...
                  alignItems={"center"}
                >
                  <Image src={selectedReel.imageUrl} alt="saved reel" />
                </Flex>
                <Flex
                  flex={1}
                  flexDir={"column"}
                ...
                >
                  ...
                  <Flex
                    ...
                    display={{ base: "none", md: "flex" }}
                  >
                    <SavedReelFooter reel={selectedReel} />
                  </Flex>
                </Flex>
              </Flex>
            )}
